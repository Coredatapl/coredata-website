...
export default function MobileNav({ opened }: MobileNavProps) {
  ...
  return (
    <div
      className={`${
        !opened ? "hidden" : "absolute top-[70px] left-0 flex flex-row"
      } w-screen h-[calc(100vh-70px)]`}
    >
      <div
        className={`${
          !visible ? "opacity-0" : "opacity-100"
        } relative flex w-screen p-10 bg-slate-100 transition-opacity duration-500 ease-in-out`}
      >
        ...
        <div className="flex flex-col w-1/2 justify-start text-sm">
          <div className="flex flex-col mb-10 max-w-40">
            <div className="font-semibold text-base">Info</div>
            <div className="mt-4">
              <MobileNavItem
                id={AppComponents.about}
                to={"/"}
